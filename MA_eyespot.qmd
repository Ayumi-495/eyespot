...
# ggsave("figS_diameter.pdf", width = 10, height = 10, dpi = 450)
```
...
summary(mr_area_T)
r2_3_1 <- round(r2_ml(mr_area_T), 4)
```
...
bubble_plot(mr_background,
            ...
            group = "Study_ID",
            xlab = "Log-transformed backgroud")
ggsave("figS_background.pdf", width = 10, height = 10, dpi = 450)
```
...
ggsave("fig-shape.pdf", width = 10, height = 10, dpi = 450)
...
```
It seems **pattern total area** is a better predictor than **diameter**?.
...
### Moderators associated with conspicuousness
Only include moderators related to conspicuousness (pattern are/total area and number of patterns) in the model.
...
summary_cons <- summary(mr_cons)
summary_cons_output <- capture.output(summary_cons)
writeLines(summary_cons_output, "summary_cons.csv")
...
p1 <- orchard_plot(ma_all,
                    ...
                    theme(legend.direction = "horizontal",
                    legend.text = element_text(size = 6)) +
                    scale_colour_manual(values = "indianred1")+
                    scale_fill_manual(values = "indianred1")

# ggsave("fig1.pdf", width = 15, height = 5, dpi = 450)
...
# ggsave("fig2.pdf", width = 15, height = 5, dpi = 450)
...
comb <- submerge(main, pattern)
# ggsave("fig3.pdf", width = 15, height = 5, dpi = 450)
comb_p <- orchard_plot(comb,
                    group = "Study_ID",
                    xlab = "log response ratio (lnRR)",
                    angle = 45,
                    branch.size = 3) +
...
# output figure as pdf
ggsave("fig1_2.pdf", width = 15, height = 7, dpi = 450)

p4 <- orchard_plot(mr_prey_shape,
                  mod = "Shape_prey",
                  group = "Study_ID",
                  xlab = "log response ratio (lnRR)",
                  angle = 45,
...
# ggsave("figS_shape.pdf", width = 15, height = 10, dpi = 450)
```
...
# diameter
p7 <- bubble_plot(mr_diameter,
            mod = "Log_diameter",
            group = "Study_ID",
            xlab = "Log-transformed diameter")

# background
p8 <- bubble_plot(mr_background,
            mod = "Log_background",
            group = "Study_ID",
...
# ggsave("figS_dimsurface.pdf", width = 10, height = 10, dpi = 450)
```
...
# Egger's test and decline effect
p9 <- bubble_plot(bias_model,
            mod = "sqrt_inv_e_n",
            group = "Study_ID",
            xlab = "Square root of inverse of effective sample size") +
            scale_y_continuous(breaks = seq(-2.5, 4.5, 1.5)) 

p10 <- bubble_plot(year_model,
            mod = "Year",
            group = "Study_ID",
            xlab = "Year of publication") +  scale_y_continuous(breaks = seq(-2.5, 4.0, 1.5)) 
...
```{r}
#| label: fig-galary4
#| fig-cap: "Publication bias (multi-moderator)"
#| fig-subcap:
#|    - "Egger's test and Decline effect (multi-moderator)"
...
p12 <- bubble_plot(multi_bias,
            mod = "Year",
            group = "Study_ID",
            xlab = "Year of publication")

p11 / p12 +  plot_annotation(tag_levels = 'a')

# ggsave("figS_pub.pdf", width = 10, height = 10, dpi = 450)
